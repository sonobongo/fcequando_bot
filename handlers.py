...
def is_metro_closed(now: datetime, station: str) -> Tuple[bool, Optional[datetime], str]:
    ...
    # Verificar intervalos especiales sin información (desde 23:59 hasta la hora de cierre)
    current_time = now.time()
    current_min = current_time.hour * 60 + current_time.minute
    
    # Nochevieja (31/12) y madrugada del 1/1 hasta las 3:00
    if is_new_years_eve(now):
        # Desde las 23:59 hasta las 3:00 (del día siguiente, pero lo tratamos en el mismo día con +24h)
        # Convertimos la hora actual a minutos desde la medianoche del día de now.
        # Para comparar con 23:59, usamos 23*60+59 = 1439. Para las 3:00, usamos 3*60 = 180, pero como es del día siguiente, sumamos 24*60=1440.
        # Si current_min >= 1439 o current_min < 180, significa que está en el intervalo.
        if current_min >= 1439 or current_min < 180:
            next_open = datetime.combine(now.date() + timedelta(days=1), time(6, 0)) if current_min >= 1439 else datetime.combine(now.date(), time(6, 0))
            next_open = CATANIA_TZ.localize(next_open)
    ...
    # Viernes y sábado: desde las 23:59 hasta la 1:00 del día siguiente
    if now.weekday() in [4, 5]:  # viernes o sábado
        # Intervalo: 23:59 hasta 1:00 (del día siguiente)
        if current_min >= 1439 or current_min < 60:
            next_open = datetime.combine(now.date() + timedelta(days=1), time(6, 0)) if current_min >= 1439 else datetime.combine(now.date(), time(6, 0))
            next_open = CATANIA_TZ.localize(next_open)
            special_msg = "🚇 Non ci sono informazioni disponibili. Ricorda che oggi l'ultima metropolitana è partita alle 01:00."
            return (True, next_open, special_msg)
    
    # Resto del código normal para determinar si está cerrado
